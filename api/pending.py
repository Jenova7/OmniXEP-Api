...
def insertbtc(rawtx):
  try:
    ...
    propertyid = 0
    txtype = 0
    txversion = rawtx['XEP']['version']
    txhash = rawtx['XEP']['txid']
    protocol = "ElectraProtocol"
    ...
    dbExecute("insert into txjson (txdbserialnum, protocol, txdata) values (%s,%s,%s)", (txdbserialnum, protocol, json.dumps(rawtx['XEP'])) )

    dbCommit()
  except Exception,e:
    print_debug(("Error: ", e, "\n Could not add XEP PendingTx: ", rawtx),2)
    dbRollback()
