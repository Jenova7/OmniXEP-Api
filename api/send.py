...
def send_form_response(response_dict):
    ...
    if 'currency' in response_dict and (response_dict['currency'] not in ['XEP','xep',0,'0']):
    ...
    if TESTNET or ('testnet' in response_dict and ( response_dict['testnet'][0] in ['true', 'True'] )):
        testnet =True
        magicbyte = 55
        exodus_address='xJZ3d5F3Z7yT31wKu93ueLcvHA2j2YSh6B'
    else:
        testnet = False
        magicbyte = 141
    ...
    #    return (None, 'From address is neither XEP address nor pubkey')
    to_addr=response_dict['to_address'][0]
    #if not is_valid_bitcoin_address(to_addr):
    #    return (None, 'To address is not a XEP address')
...
# simple send and XEP send (with or without marker)
def prepare_send_tx_for_signing(from_address, to_address, marker_address, amount, btc_fee=500000, magicbyte=0):
    ...
    if change_value < 0:
        info('Error not enough XEP to generate tx - negative change')
        raise Exception('This address must have enough XEP for tx fees and protocol transaction fees')

    # create a normal XEP transaction (not mastercoin)
